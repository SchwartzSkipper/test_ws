...
namespace yocs {
                                     ...
                                     yocs_msgs::TrajectoryList& trajs) : nh_(n)
  {
    ...
    waypoint_add_sub_ = nh_.subscribe("waypoint_add", 100, &WaypointProvider::waypoint_add_sub, this);
    waypoint_remove_sub_ = nh_.subscribe("waypoint_remove", 100, &WaypointProvider::waypoint_remove_sub, this);
    waypoint_revise_sub_ = nh_.subscribe("waypoint_revise", 100, &WaypointProvider::waypoint_revise_sub, this);
    trajectory_add_sub_ = nh_.subscribe("trajectory_add", 100, &WaypointProvider::trajectory_add_sub, this);
    trajectory_remove_sub_ = nh_.subscribe("trajectory_remove", 100, &WaypointProvider::trajectory_remove_sub, this);
  ...
  void WaypointProvider::spin() {
      ...
      std::string filename;
      priv_n.getParam("filename", filename);
      yocs::saveWaypointsAndTrajectoriesToYaml(filename, waypoints_, trajectories_);
  ...
  void WaypointProvider::waypoint_add_sub(const yocs_msgs::Waypoint::ConstPtr& wp)
  {
      std::vector<yocs_msgs::Trajectory>::iterator traj_iter;
      for (traj_iter = trajectories_.trajectories.begin(); traj_iter != trajectories_.trajectories.end(); traj_iter++)
      {
          std::vector<yocs_msgs::Waypoint>::iterator wp_iter;
          for (wp_iter = traj_iter->waypoints.begin(); wp_iter != traj_iter->waypoints.end(); wp_iter++)
          {
              if (wp_iter->name == wp->name)
              {
                  traj_iter->waypoints.erase(wp_iter);
                  wp_iter = traj_iter->waypoints.insert(wp_iter, *wp);
              }
          }
          if (traj_iter->name == wp->name)
          {
              trajectories_.trajectories.erase(traj_iter);
              traj_iter--;
          }
      }

      std::vector<yocs_msgs::Waypoint>::iterator wp_iter;
      for (wp_iter = waypoints_.waypoints.begin(); wp_iter != waypoints_.waypoints.end(); wp_iter++)
      {
          if (wp_iter->name == wp->name)
          {
              waypoints_.waypoints.erase(wp_iter);
              wp_iter--;
          }
      }
  ...
  void WaypointProvider::waypoint_remove_sub(const yocs_msgs::Waypoint::ConstPtr& wp)
  {
      std::vector<yocs_msgs::Trajectory>::iterator traj_iter;
      for (traj_iter = trajectories_.trajectories.begin(); traj_iter != trajectories_.trajectories.end(); traj_iter++)
      {
          ...
          if (traj_iter->name == wp->name || traj_iter->waypoints.empty())
          {
              trajectories_.trajectories.erase(traj_iter);
              traj_iter--;
          }
  ...
  void WaypointProvider::waypoint_revise_sub(const yocs_msgs::Waypoint::ConstPtr& wp)
  {
      std::vector<yocs_msgs::Waypoint>::iterator wp_iter;
      for (wp_iter = waypoints_.waypoints.begin(); wp_iter != waypoints_.waypoints.end(); wp_iter++)
      {
          if (wp_iter->name == wp->name)
          {
              wp_iter->header.frame_id = wp->header.frame_id;
              wp_iter->pose = wp->pose;
          }
      }
  ...
  void WaypointProvider::trajectory_add_sub(const yocs_msgs::Trajectory::ConstPtr& traj)
  {
      std::vector<yocs_msgs::Waypoint>::iterator wp_iter;
      for (wp_iter = waypoints_.waypoints.begin(); wp_iter != waypoints_.waypoints.end(); wp_iter++)
      {
          if (wp_iter->name == traj->name)
          {
              waypoints_.waypoints.erase(wp_iter);
              wp_iter--;
          }
  ...
  void WaypointProvider::trajectory_remove_sub(const yocs_msgs::Trajectory::ConstPtr& traj)
  {
      std::vector<yocs_msgs::Waypoint>::iterator wp_iter;
      for (wp_iter = waypoints_.waypoints.begin(); wp_iter != waypoints_.waypoints.end(); wp_iter++)
      {
